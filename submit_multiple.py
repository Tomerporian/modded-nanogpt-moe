...
if __name__ == '__main__':
    ...
    if args.num_configs:
        ...
        configs = configs[args.num_configs:] # need to run with -n 7818
    ...
    for spec_name, spec_filename, out_dir in gen():
        ...
        cmd = f'sbatch --job-name={spec_name} --output={out_path} --error={out_path} {args.script} {specs}'
        if args.dry_run:
            # print(f'Would now run "{cmd}"')
            if args.rerun:
                # with open(os.path.join(out_dir, 'config.yaml'), 'r') as f:
                #     prev_config = yaml.safe_load(f)
                print(f'previous failed run: {out_dir}')
        else:
            while True:
                try:
